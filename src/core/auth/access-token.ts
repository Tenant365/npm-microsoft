...
export interface M365LocalCertificateSigningAuthenticationRequest {
  tenantId: string;
  clientId: string;
  privateKey: string | CryptoKey;
  certificate: string;
...
export const getM365AccessTokenWithLocalCertificateSigning = async (
  request: M365NodeSigningAccessTokenRequest,
): Promise<M365AccessToken> => {
  const auth = createM365ClientCertificate({
    tenantId: request.tenantId,
...
export const getM365AuthenticationWithLocalCertificateSigning = async (
  request: M365LocalCertificateSigningAuthenticationRequest,
): Promise<M365Authentication> => {
  return createM365ClientCertificate({
    tenantId: request.tenantId,
    clientId: request.clientId,
...
export class M365AuthenticationProvider {
  public async buildWithKeyVaultSigning(
    request: M365KeyVaultSigningAccessTokenRequest,
  ): Promise<M365Authentication> {
    return await getM365AuthenticationWithKeyVaultSigning(request);
  }

  public async buildWithLocalCertificateSigning(
    request: M365LocalCertificateSigningAuthenticationRequest,
  ): Promise<M365Authentication> {
    return await getM365AuthenticationWithLocalCertificateSigning(request);
  }
}
